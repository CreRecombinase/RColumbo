...
%   $Revision: 1.1.6.1 $  $Date: 2007/05/23 19:15:16 $

% Check square, symmetric, positive semidefinite.
if nargin < 2
    [T,p] = cholcov(C);
    if p ~= 0
        error('stats:corrcov:BadC', ...
              'C must be a square, symmetric, positive semidefinite matrix.');
    end
end

[m,n] = size(C);
sigma = sqrt(diag(C)); % sqrt first to avoid under/overflow
% R = bsxfun(@rdivide,C,sigma); R = bsxfun(@rdivide,R,sigma');
R = C ./ sigma*sigma';
