...
run_RSS <- function(param_snpfile,param_expfile,snp_chunk,exp_chunk,tissue,
  ...
  eqtl_snplist <-read_vec(param_snpfile,paste0(snp_chunk,"/Adipose_Subcutaneous"))
  ...
  eqtl_explist <-read_vec(param_expfile,paste0(exp_chunk,"/Adipose_Subcutaneous"))
  eqtl_expdat <- read_ind_h5(eqtl_expfile,"EXPdata","expression",eqtl_explist)
  exp_fgeneid <- read_df_h5(eqtl_expfile,"EXPinfo",subcols = "fgeneid",filtervec = eqtl_explist)[["fgeneid"]]
  ...
  for(i in 1:length(summ_statl)){
    ...
    fit_mat <- grid_search_rss_varbvsr(SiRiS = SiRiS,sigma_beta = sigb,logodds=logodds,betahat = betahat,
                                                                      se = se,talpha0 = alpha0,tmu0 = mu0,tSiRiSr0 = SiRiSr@x,tolerance = 1e-3,itermax = 100,verbose = T,lnz_tol = T)
    ret_sigb <-sort(rep(sigb,length(logodds)))
    ret_logodds <- rep(logodds,length(sigb))
    fit_dfl[[i]] <- data_frame(logodds=ret_logodds,sigb=ret_sigb,lnZ=c(fit_mat)) %>% mutate(fgeneid=exp_fgeneid[i])
    gc()
  }
  fit_df <- bind_rows(fit_dfl) %>% mutate(snp_chunk=snp_chunk,exp_chunk=exp_chunk)
  return(fit_df)
}
